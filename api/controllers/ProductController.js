...
module.exports = {
    getAll: async (req, res) => {
        ...
        try {
          ...
          const count = await M_Product.count(query);
          if (count > 0) {
            labs = await M_Product.find(query)
      ...
      getOne: async (req, res) => {
        ...
        try {
          console.log("ID : ",id)
          const data = await M_Product.findOne({
            where: {
              id: id,
              dtmDeletedAt: null,
            },
      ...
      getCode: async (req, res) => {
        try {
          const labs = await M_Product.find({
            where : {
              dtmDeletedAt : null
            },
            select : ["id","txtName"]
      ...
      create: async (req, res) => {
        ...
        let { body } = req;
        try {
          const data = await M_Product.create({
            txtName: body.name,
      ...
      update: async (req, res) => {
        ...
        let { body } = req;
        try {
          const lab = await M_Product.findOne({
            where: {
              id: params.id,
              dtmDeletedAt: null,
            },
          ...
          const data = await M_Product.update({
            id: params.id,
          }).set({
            txtName: body.name,
