...
const M_Shift = require("../models/M_Shift");
...
module.exports = {
  getAll: async (req, res) => {
    ...
    try {
      ...
      const count = await M_Shift.count(query);
      if (count > 0) {
        shift = await M_Shift.find(query)
  ...
  getOne: async (req, res) => {
    const { id } = req.params;
    try {
      const data = await M_Shift.findOne({
        where: {
          id: id,
          dtmDeletedAt: null,
        },
  ...
  create: async (req, res) => {
    const { user, body } = req;
    try {
      const data = await M_Shift.create({
        txtName: body.name,
        // txtCreatedBy: user.id,
        dtmJadwal: body.jadwal,
      }).fetch();
  ...
  update: async (req, res) => {
    ...
    try {
      ...
      if (!shift) {
        sails.helpers
          .errorResponse("shift not found", "failed")
          .then((resp) => {
            res.status(401).send(resp);
          });
      }

      const data = await M_Shift.update({
        id: params.id,
      }).set({
        txtName: body.name,
        dtmJadwal: body.jadwal,
